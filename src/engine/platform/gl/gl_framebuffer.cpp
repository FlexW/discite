...
void GlFramebuffer::attach(const FramebufferConfig &config)
{
       ...
       ++i)
  {
            ...
            attachment_config))
    {
      ...
      case AttachmentType::Texture:
      {
        ...
        texture_config.sized_format_ = color_attachment.sized_format_;
        texture_config.wrap_s_       = TextureWrapMode::Repeat;
        texture_config.wrap_t_       = TextureWrapMode::Repeat;
        texture_config.min_filter_   = TextureFilterMode::Linear;
        texture_config.mag_filter_   = TextureFilterMode::Linear;
      ...
      {
        RenderbufferConfig renderbuffer_config{};
        renderbuffer_config.sized_format_ = color_attachment.sized_format_;
    ...
    else if (std::holds_alternative<std::shared_ptr<TextureArray>>(
                 attachment_config))
    {
      const auto tex_array = std::dynamic_pointer_cast<GlTextureArray>(
          std::get<std::shared_ptr<TextureArray>>(attachment_config));
  ...
  if (config.depth_attachment_.has_value())
  {
            ...
            attachment_config))
    {
      ...
      case AttachmentType::Texture:
      {
        ...
        texture_config.sized_format_ = depth_attachment.sized_format_;
        texture_config.wrap_s_       = TextureWrapMode::Repeat;
        texture_config.wrap_t_       = TextureWrapMode::Repeat;
        texture_config.min_filter_   = TextureFilterMode::Linear;
        texture_config.mag_filter_   = TextureFilterMode::Linear;
      ...
      {
        RenderbufferConfig renderbuffer_config{};
        renderbuffer_config.sized_format_ = depth_attachment.sized_format_;
                 ...
                 attachment_config))
    {
      const auto tex_array =
          std::dynamic_pointer_cast<GlTextureArray>(std::get<std::shared_ptr<TextureArray>>(attachment_config));
      glNamedFramebufferTexture(id_, GL_DEPTH_ATTACHMENT, tex_array->id(), 0);
      depth_attachment_ = tex_array;
    }
    else if (std::holds_alternative<std::shared_ptr<CubeTexture>>(
                 attachment_config))
    {
      const auto cube_texture =
          std::dynamic_pointer_cast<GlCubeTexture>(std::get<std::shared_ptr<CubeTexture>>(attachment_config));
      glNamedFramebufferTexture(id_,
                                GL_DEPTH_ATTACHMENT,
                                cube_texture->id(),
                                0);
  ...
  if (config.stencil_attachment_.has_value())
  {
            ...
            attachment_config))
    {
      ...
      {
        RenderbufferConfig renderbuffer_config{};
        renderbuffer_config.sized_format_ = stencil_attachment.sized_format_;
...
void GlFramebuffer::set_color_attachment(std::size_t                  index,
                                         ...
                                         GLuint                       mip)
{
  const auto gl_texture = std::dynamic_pointer_cast<GlCubeTexture>(texture);
...
{
  const auto gl_renderbuffer =
      std::dynamic_pointer_cast<GlRenderbuffer>(renderbuffer);
  glNamedFramebufferRenderbuffer(id_,
                                 GL_DEPTH_ATTACHMENT,
                                 gl_renderbuffer->id(),
                                 0);
...
void GlFramebuffer::set_depth_attachment(std::shared_ptr<CubeTexture> value)
{
  const auto gl_texture = std::dynamic_pointer_cast<GlTexture>(value);
  glNamedFramebufferTexture(id_, GL_DEPTH_ATTACHMENT, gl_texture->id(), 0);
