...
GlCubeTexture::GlCubeTexture(const CubeTextureConfig &config)
{
  ...
  glTextureParameteri(id_, GL_TEXTURE_WRAP_R, to_gl(config.wrap_r_));
  ...
  {
    glTextureParameteri(id_, GL_TEXTURE_MIN_FILTER, to_gl(config.min_filter_));
  }
